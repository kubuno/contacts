...
export default function ContactsSidebarBody({ collapsed = false }: { collapsed?: boolean }) {
  ...
  async function delLabel(e: React.MouseEvent, id: string) {
    e.stopPropagation()
  ...
  async function delGroup(e: React.MouseEvent, id: string) {
    e.stopPropagation()
  ...
  const Nav = (p: { label: string; icon: React.ReactNode; active: boolean; onClick: () => void; badge?: number }) =>
    ...
    <>
      {collapsed ? (
        <div className="flex justify-center mb-3">
          <button onClick={() => setEditorOpen(true)} title={t('create_contact')}
      ...
      <nav className={`flex-1 overflow-y-auto space-y-0.5 ${collapsed ? 'px-2' : 'px-3'}`}>
        <Nav label={t('title_all')} icon={<Users size={20} />} active={view === 'all'} onClick={() => setView('all')} badge={total} />
        <Nav label={t('title_starred')} icon={<Star size={20} />} active={view === 'starred'} onClick={() => setView('starred')} />
        ...
        <Nav label={t('nav_birthdays')} icon={<Cake size={20} />} active={view === 'birthdays'} onClick={() => setView('birthdays')} />
        <Nav label={t('nav_reminders')} icon={<Bell size={20} />} active={view === 'reminders'} onClick={() => setView('reminders')} badge={dueCount || undefined} />
        <Nav label={t('nav_frequent')} icon={<Clock size={20} />} active={view === 'frequent'} onClick={() => setView('frequent')} />
        <Nav label={t('nav_followup')} icon={<RotateCcw size={20} />} active={view === 'followup'} onClick={() => setView('followup')} />

        {/* Manage */}
        {collapsed ? <div className="mx-1 my-1 h-px bg-border" /> : (
          <div className="pt-4 pb-1 px-1"><span className="text-xs font-semibold text-text-secondary">{t('manage')}</span></div>
        )}
        <Nav label={t('title_duplicates')} icon={<GitMerge size={20} />} active={view === 'duplicates'} onClick={() => setView('duplicates')} />
        <Nav label={t('import')} icon={<Download size={20} />} active={false} onClick={() => setImportOpen(true)} />
        <Nav label={t('nav_archived')} icon={<Archive size={20} />} active={view === 'archived'} onClick={() => setView('archived')} />
        <Nav label={t('title_trashed')} icon={<Trash2 size={20} />} active={view === 'trashed'} onClick={() => setView('trashed')} />
        <Nav label={t('nav_settings')} icon={<Settings size={20} />} active={view === 'settings'} onClick={() => setView('settings')} />
        ...
        {!collapsed && (
          <>
            <div className="pt-4 pb-1 px-1 flex items-center justify-between">
              ...
              <button onClick={() => setShowNewLabel(v => !v)} className="p-1 rounded-full hover:bg-surface-2 text-text-secondary"><Plus size={14} /></button>
            </div>
            {showNewLabel && (
              <div className="px-1 mb-2">
                ...
                <button onClick={createLabel} className="text-xs text-primary hover:underline">{t('create')}</button>
              </div>
            )}
            {labels.map(l => {
              const active = view === 'label' && activeLabelId === l.id
              return (
                <button key={l.id} onClick={() => setView('label', l.id)}
                  className={`w-full flex items-center gap-3 px-3 py-2 rounded-full text-sm transition-colors group ${active ? 'bg-primary-light text-primary' : 'text-text-secondary hover:bg-surface-2'}`}>
                  ...
                  <button onClick={e => delLabel(e, l.id)} className="hidden group-hover:flex items-center text-text-secondary hover:text-danger"><Trash size={12} /></button>
                </button>
              )
            })}
            ...
            {groups.map(g => {
              ...
              return (
                <button key={g.id} onClick={() => setView('group', g.id)}
                  className={`w-full flex items-center gap-3 px-3 py-2 rounded-full text-sm transition-colors group ${active ? 'bg-primary-light text-primary' : 'text-text-secondary hover:bg-surface-2'}`}>
                  ...
                  <button onClick={e => delGroup(e, g.id)} className="hidden group-hover:flex items-center text-text-secondary hover:text-danger"><Trash size={12} /></button>
                </button>
              )
            })}
          </>
