...
use clap::Parser;
use kubuno_contacts::{config::Settings, router, state::AppState};
use reqwest::Client;
...
#[derive(Parser, Debug)]
#[command(name = "kubuno-contacts", version, about = "Module contacts Kubuno")]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    match settings.logging.format {
        kubuno_contacts::config::LogFormat::Json   => subscriber.json().init(),
        kubuno_contacts::config::LogFormat::Pretty => subscriber.init(),
    }

    tracing::info!("Kubuno Contacts v{} démarrage…", env!("CARGO_PKG_VERSION"));

    // Sécurité : interdire toute exécution de processus sur l’hôte (voir kubuno-seccomp).
    kubuno_seccomp::lock_down_process_execution("contacts");
    ...
    if settings.database.run_migrations {
        sqlx::query("CREATE SCHEMA IF NOT EXISTS contacts")
            .execute(&pool)
            .await
            .context("Création du schéma contacts")?;

        let migration_opts = settings.database.connect_options()?
            .options([("search_path", "contacts,public")]);
    ...
    // One client, shared by the state (directory resolution) and the
    // registration/heartbeat loop below.
    let http = Client::new();

    // Instance settings: compiled defaults, then one read from the core so the
    // quotas, the sharing policy and the CardDAV switch see the administrator's
    // values from the first request on.
    let instance = Arc::new(std::sync::RwLock::new(
        kubuno_contacts::config::instance::InstanceConfig::default(),
    ));
    if let Some(cfg) = kubuno_contacts::config::instance::fetch(
    ...
    // Heartbeat toutes les 30s
    {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let url    = format!("{}/internal/modules/contacts/heartbeat", settings2.core.url);
    ...
    tracing::info!("Kubuno Contacts démarré sur http://{addr}");
...
async fn register_with_core(http: &Client, settings: &Settings) {
    ...
    let display_name  = manifest.as_ref().map(|m| m.module.display_name.as_str()).unwrap_or("Contacts").to_string();
    ...
    let sidebar_items: Vec<Value> = manifest.as_ref()
        ...
        .unwrap_or_else(|| vec![
            json!({ "id": "contacts", "label": "Contacts", "icon": "Users", "path": "/contacts", "position": 40 }),
        ]);
    let subscribed_events: Vec<String> = manifest.as_ref()
        .and_then(|m| m.events.as_ref())
        .map(|e| e.subscribed.clone())
        .unwrap_or_else(|| vec!["UserCreated".into(), "UserUpdated".into(), "UserDeleted".into()]);
    ...
    for attempt in 1u32.. {
            ...
            .await
        {
            Ok(resp) if resp.status().is_success() => {
                tracing::info!("Module contacts enregistré auprès du core");
                return;
            }
